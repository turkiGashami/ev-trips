...
export function CityAutocomplete({
  ...
  const select = (city: any) => {
    setInputValue(city.name_ar ?? city.nameAr ?? city.name);
    onSelect(city.id, city.name_ar ?? city.nameAr ?? city.name, city.name ?? city.nameEn);
    setOpen(false);
    setHighlighted(-1);
  };
        ...
        >
          {suggestions.map((city: any, i: number) => (
            ...
            >
              ...
              <span className="font-medium">{city.name_ar ?? city.nameAr ?? city.name}</span>
              <span className="text-[var(--ink-3)] text-xs ms-auto">{city.name ?? city.nameEn}</span>
            </li>
          ))}
        </ul>
      )}
