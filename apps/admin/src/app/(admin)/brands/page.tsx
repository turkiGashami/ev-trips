...
function AddModelForm({ brandId, onCancel, onCreated }: { brandId: string; onCancel: () => void; onCreated: () => void }) {
  ...
  const submit = async () => {
    setError(null);
    if (!name.trim()) { setError('الاسم بالإنجليزية مطلوب'); return; }
    setSaving(true);
    try {
      ...
      setError(err?.response?.data?.message ?? 'تعذّر إضافة الموديل');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    ...
    >
      <div>
        <label className="form-label" style={{ fontSize: 11, color: 'var(--ink-3)', marginBottom: 4, display: 'block' }}>الاسم (إنجليزي)</label>
        <input className="form-input" dir="ltr" value={name} onChange={(e) => setName(e.target.value)} placeholder="Model Y" />
      </div>
      <div>
        <label className="form-label" style={{ fontSize: 11, color: 'var(--ink-3)', marginBottom: 4, display: 'block' }}>الاسم (عربي)</label>
        <input className="form-input" value={nameAr} onChange={(e) => setNameAr(e.target.value)} placeholder="موديل Y" />
      </div>
      <div>
        <label className="form-label" style={{ fontSize: 11, color: 'var(--ink-3)', marginBottom: 4, display: 'block' }}>المعرّف (slug)</label>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        <button onClick={submit} disabled={saving}
          ...
          <Check style={{ width: 13, height: 13 }} /> حفظ
        </button>
        <button onClick={onCancel} disabled={saving}
          ...
          <X style={{ width: 13, height: 13 }} /> إلغاء
        </button>
      </div>
  ...
  return (
    <div style={{ padding: '10px 48px 14px', background: 'var(--sand)' }} dir="rtl">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        ...
        {!adding && (
          <button onClick={() => setAdding(true)}
            ...
            <Plus style={{ width: 13, height: 13 }} /> إضافة موديل
          </button>
        )}
      </div>

      {isLoading ? (
        <p style={{ fontSize: 12, color: 'var(--ink-4)' }}>جارٍ التحميل...</p>
      ) : models.length === 0 && !adding ? (
        <p style={{ fontSize: 12, color: 'var(--ink-4)', padding: '6px 0' }}>لا توجد موديلات بعد</p>
      ) : (
          ...
          {models.map((model: any) => (
            <div key={model.id}
              ...
              <span style={{ marginInlineStart: 'auto', fontSize: 11, color: 'var(--ink-4)' }}>
                {formatNumber(model.trims_count ?? 0)} فئة
              </span>
      ...
      {confirmDeleteId && (
        <ConfirmModal
          title="حذف الموديل"
          description="سيُحذف هذا الموديل وكل فئاته (trims). لا يمكن التراجع."
          confirmLabel="حذف"
          variant="danger"
...
function AddBrandModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.message ?? 'تعذّر إضافة الماركة');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      ...
      <div style={{ position: 'relative', width: '100%', maxWidth: 480, background: 'var(--cream)', border: '1px solid var(--line)', borderRadius: 4, padding: 24 }} dir="rtl">
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
          <h3 className="heading-3">إضافة ماركة جديدة</h3>
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          <div>
            <label className="form-label" style={{ fontSize: 12, color: 'var(--ink-3)', marginBottom: 4, display: 'block' }}>الاسم *</label>
            <input className="form-input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Tesla" dir="ltr" />
          </div>
          <div>
            <label className="form-label" style={{ fontSize: 12, color: 'var(--ink-3)', marginBottom: 4, display: 'block' }}>المعرّف (slug)</label>
            <input className="form-input" value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="tesla" dir="ltr" />
          </div>
          <div>
            <label className="form-label" style={{ fontSize: 12, color: 'var(--ink-3)', marginBottom: 4, display: 'block' }}>رابط الشعار</label>
...
export default function AdminBrandsPage() {
  ...
  return (
    <>
      <AdminTopbar title="الماركات والموديلات" subtitle={`${formatNumber(brands.length)} ماركة`} />
      <main className="admin-main" dir="rtl">
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16 }}>
          <button onClick={() => setAddingBrand(true)}
            ...
            <Plus style={{ width: 14, height: 14 }} /> إضافة ماركة
          </button>
        </div>
        ...
        {isLoading ? (
          ...
          <div style={{ textAlign: 'center', padding: '48px 16px', background: 'var(--cream)', border: '1px solid var(--line)', borderRadius: 4 }}>
            <p style={{ color: 'var(--ink-4)', fontSize: 13 }}>لا توجد ماركات بعد</p>
          </div>
        ) : (
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 18px' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ fontSize: 11, color: 'var(--ink-4)', marginTop: 2 }}>
                        {formatNumber(brand.models_count ?? 0)} موديل
