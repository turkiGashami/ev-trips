    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      ...
      <div style={{ position: 'relative', width: '100%', maxWidth: 640, background: 'var(--cream)', border: '1px solid var(--line)', borderRadius: 4, boxShadow: '0 20px 40px rgba(0,0,0,0.15)', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid var(--line)' }}>
          <h3 style={{ fontSize: 14, fontWeight: 500, color: 'var(--ink)' }}>{station ? "تعديل محطة الشحن" : "إضافة محطة شحن جديدة"}</h3>
        ...
        <form onSubmit={handleSubmit} id="station-form" style={{ overflowY: 'auto', flex: 1 }} dir="rtl">
          <div style={{ padding: '16px 20px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            {field("الاسم بالإنجليزية", <input type="text" required placeholder="Station Name" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} className="form-input" dir="ltr" />)}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {field("إجمالي المنافذ", <input type="number" placeholder="8" value={form.totalSlots} onChange={(e) => setForm((f) => ({ ...f, totalSlots: e.target.value }))} className="form-input" dir="ltr" />)}
              {field("المنافذ المتاحة", <input type="number" placeholder="4" value={form.availableSlots} onChange={(e) => setForm((f) => ({ ...f, availableSlots: e.target.value }))} className="form-input" dir="ltr" />)}
            </div>
            <div style={{ gridColumn: '1 / -1' }}>
              <label className="form-label">أنواع الموصلات</label>
            ...
            {field("أوقات العمل", <input type="text" placeholder="24/7 أو 8 صباحاً - 10 مساءً" value={form.openHours} onChange={(e) => setForm((f) => ({ ...f, openHours: e.target.value }))} className="form-input" />)}
            {field("رقم الهاتف", <input type="tel" placeholder="+966 5xxxxxxxx" value={form.phone} onChange={(e) => setForm((f) => ({ ...f, phone: e.target.value }))} className="form-input" dir="ltr" />)}
            <div style={{ gridColumn: '1 / -1' }}>
              {field("الموقع الإلكتروني", <input type="url" placeholder="https://..." value={form.website} onChange={(e) => setForm((f) => ({ ...f, website: e.target.value }))} className="form-input" dir="ltr" />)}
            </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 8, padding: '12px 20px', borderTop: '1px solid var(--line)' }}>
          <AdminButton variant="ghost" onClick={onClose} disabled={isSubmitting}>إلغاء</AdminButton>
          <AdminButton variant="primary" type="submit" form="station-form" isLoading={isSubmitting}>{station ? "حفظ التعديلات" : "إضافة المحطة"}</AdminButton>
        </div>
      </div>
    </div>
  );
...
export default function ChargingStationsPage() {
  ...
  const columns: Column<ChargingStation>[] = [
    ...
    { key: "connectorTypes", header: "أنواع الشحن", render: (s) => <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3 }}>{s.connectorTypes?.map((c) => <ConnectorBadge key={c} type={c} />)}</div> },
    { key: "maxPowerKw", header: "الطاقة (كيلوواط)", render: (s) => <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, color: 'var(--ink-3)' }}><Zap style={{ width: 12, height: 12, color: '#d97706' }} />{formatNumber(s.maxPowerKw)} kW</div> },
    { key: "slots", header: "المنافذ", render: (s) => <span style={{ fontSize: 12, color: 'var(--ink-3)' }}>{formatNumber(s.availableSlots)} / {formatNumber(s.totalSlots)}</span> },
    {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          {s.isVerified && (
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, padding: '2px 8px', fontSize: 10, fontWeight: 500, background: 'rgba(107,142,156,.12)', color: 'var(--sky)', borderRadius: 2 }}>
              <CheckCircle style={{ width: 10, height: 10 }} />موثقة
            </span>
          )}
        </div>
      ),
    },
    {
      ...
      render: (s) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: 2, justifyContent: 'flex-end' }}>
          <button onClick={() => setFormModal({ open: true, station: s })} style={iconBtnStyle} title="تعديل"><Pencil style={{ width: 14, height: 14 }} /></button>
          <button onClick={() => toggleMutation.mutate(s.id)} style={iconBtnStyle} title={s.isActive ? "تعطيل" : "تفعيل"}>
            {s.isActive ? <XCircle style={{ width: 14, height: 14 }} /> : <CheckCircle style={{ width: 14, height: 14 }} />}
          </button>
          <button onClick={() => setDeleteTarget(s)} style={{ ...iconBtnStyle, color: 'var(--terra)' }} title="حذف"><Trash2 style={{ width: 14, height: 14 }} /></button>
        </div>
      ),
    },
  ];
  ...
  return (
    <>
      <AdminTopbar title="محطات الشحن" subtitle={`${formatNumber(total)} محطة`} />
      <main className="admin-main" dir="rtl">
        {isError && (
          <div style={{ background: 'rgba(180,94,66,0.08)', border: '1px solid rgba(180,94,66,0.3)', borderRadius: 3, padding: '10px 14px', fontSize: 13, color: 'var(--terra)', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
            <AlertTriangle style={{ width: 14, height: 14, flexShrink: 0 }} /> حدث خطأ أثناء تحميل المحطات.
          </div>
        )}
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, marginBottom: 16 }}>
          <div style={{ position: 'relative', flex: '1 1 200px', maxWidth: 320 }}>
            ...
            <input type="text" placeholder="البحث عن محطة…" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} dir="rtl" className="form-input" style={{ paddingRight: 32 }} />
          ...
          <AdminButton variant="primary" leftIcon={<Plus style={{ width: 14, height: 14 }} />} onClick={() => setFormModal({ open: true, station: null })}>إضافة محطة</AdminButton>
        </div>

        <DataTable columns={columns} data={stations} isLoading={isLoading} page={page} totalPages={totalPages} onPageChange={setPage} total={total} limit={PAGE_LIMIT} emptyMessage="لا توجد محطات شحن مسجلة" />
      </main>
      ...
      {deleteTarget && (
        <ConfirmModal isOpen onClose={() => setDeleteTarget(null)} onConfirm={() => deleteMutation.mutate(deleteTarget.id)}
          title="حذف المحطة" message={`هل تريد حذف محطة "${deleteTarget.nameAr || deleteTarget.name}" نهائياً؟`}
          confirmLabel="حذف" cancelLabel="إلغاء" variant="danger" isLoading={deleteMutation.isPending} />
      )}
    </>
  );
}
